...
\end{gather*}
% 
\noindent La funzione di attivazione $f(v)$ del percettrone è la funzione \textsl{``segno''}\,(Figura\,\ref{fig:sign-function}), chiamata anche \textsl{gradino bipolare} e definita come segue\,\cite{flasinski2016introduction}. 
% 
\begin{figure}[!t]
    \centering
...
Oltre a differire per la learning rule, alcuni modelli differiscono per l'activation function. Oltre alla funzione ``segno'' anche la funzione \textsl{sigmoide} (Figura\,\ref{fig:sigmoid-function}) è largamente utilizzata.
% 
\begin{gather*}
...
\end{figure}
% 
In particolare, data una rete neurale, i neuroni del livello $r$ ricevono il segnale dai neuroni del livello $r-1$ e, dopo aver elaborato le informazioni, inviano il segnale processato i neuroni del livello $r+1$. Osservando la Figura\,\ref{fig:neural-network} si introduce una nuova notazione per le reti neurali: si definisce con $N^{(r)(k)}$ il neurone $k$-esimo che si trova nel livello $r$; di conseguenza il suo output è identificato dalla notazione $y^{(r)(k)}$ e il suo input è $X^{(r)(k)}_i$, con $i\,\in\,\left[1,\,n\right]$, associato al peso $w^{(r)(k)}_i$. Si osserva che $n$ è il numero di input del neurone, e di conseguenza è il numero di neuroni presenti nel livello precedente; particolarmente l'input $i$-esimo di un neurone generico $k$ in un livello $r$ è uguale all'output del neurone $k$ al livello $r-1$:
% 
\begin{gather*}
    y^{(r-1)(i)} = X^{(r)(k)}_i
\end{gather*}
% 
\noindent Utilizzando la somma algebrica, illustrata nell'equazione\,\ref{eq:algebric-sum}, l'output di un neurone $k$, in un livello $r$ è dato da:
% 
\begin{gather*}
    y^{(r)(k)} = f \left( \sum_i\,w_i^{(r)(k)}\, X_i^{(r)(k)}\right) = f \left( \sum_i\,w_i^{(r)(k)}\, y^{(r-1)(i)}\right)
\end{gather*}
% 
\noindent Tale risultato può essere osservato graficamente nella Figura.

\cite{flasinski2016introduction}


Come nel caso di un singolo neurone artificiale, anche una rete neurale deve essere allenata. Per allenare una \acs{NN} i pesi della rete devono essere impostati in modo tale da \textsl{minimizzare} l'errore dato un determinato dataset di partenza. I pesi vengono man mano rifiniti durante la fase di allenamento attraverso la derivata parziale della funzione ``errore'' calcolate sui pesi. Questo approccio è del tutto analogo all'approccio del \textit{Gradient Descent} (\acs{GD})



si utilizza l'algoritmo della \textit{backpropagation}, introdotto per la prima volta nel 1986. L'idea alla base di questo algoritmo è quella di minimizzare una funzione di errore attraverso l'approccio del \textit{Gradient Descent} (\acs{GD}). Essendo che questo approccio richiede il calcolo del gradiente, è necessario che la funzione di attivazione sia continua e differenziabile: a questo proposito si utilizza la funzione sigmoide (Figura\,\ref{fig:sigmoid-function}) anziché la funzione ``segno'' adottata nel percettrone.
...
\begin{gather*}
    ...
    \hspace{50px}
    % 
    \frac{d}{dx}s(x) = \frac{e^{-x}}{{\left( 1 + e^{-x} \right)}^2} = s(x)\left(1 - s(x)\right)
\end{gather*}
% 
\noindent Per una rete compota da percettroni, la funzione ``segno'' può essere approssimata dalla \textsl{sigmoide simmetrica}, che non è altro che la tangente iperbolica di $\frac{x}{2}$:
% 
\begin{gather*}
...
\,\cite{flasinski2016introduction, rojas1996backpropagation}
